...
function configSigmaElements(config) {
    ...
    var a = [],party=['Socialdemokratiet', 'Konservative Folkeparti','Radikale Venstre','Venstre', 'Socialistisk Folkeparti', 'Enhedslisten', 'Liberal Alliance', 'Folkebevægelsen mod EU', 'Outside the Parliament', 'Dansk Folkeparti', 'Alternativet', 'Javnadarflokkurin', 'Siumut', 'Inuit Ataqatigiit',  'Tjodveldi', 'Sambandsflokkurin'],
        b,x=0;
		for (b in sigInst.clusters) a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div> ' + (party[x++]) + ' (' + sigInst.clusters[b].length + ' members)</a></div>');
    //a.sort();
